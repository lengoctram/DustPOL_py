...
for los in los_range:
    exe.isoProtostar_los(
                    los,
                    progress=True,
                    save_output=True,
                    filename_output=f"protostar_r0={los/constants.pc:.2f}pc_amax={amax:.2f}"
                    )

## Plotting the results
output_abs = [
    f"{exe.output_dir}/protostar_r0={los/constants.pc:.2f}pc_amax={amax:.2f}_abs.dat"
    for los in los_range
]
output_emi = [
    f"{exe.output_dir}/protostar_r0={los/constants.pc:.2f}pc_amax={amax:.2f}_emi.dat"
    for los in los_range
]
