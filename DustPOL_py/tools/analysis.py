...
def get_NH(filename):
	...
	lines=f.readlines()
	f.close()
	NH_str = lines[9].split(',')
	NH_ls=[]
	for i in range(len(NH_str)):
		try:
			NH_ls.append(eval(NH_str[i]))
		except:
			redo=NH_str[i].split()
			for j in range(len(redo)):
				try:
					NH_ls.append(eval(redo[j]))
				except:
					continue
	NH_=np.array(NH_ls)
...
def plot_pl_single_files(filenames,ax,**plot_kwargs):
	...
	for i,filename in enumerate(filenames):
		...
		data_=np.loadtxt(filename,skiprows=12)
...
def plot_pl(filenames,av_range=None,ax=None,**plot_kwargs):
	...
	##get the data
	# data=ascii.read(filename,header_start=7, include_names=names)
	data_=np.loadtxt(filenames,skiprows=12)
...
@auto_refresh
def plot_pav(filenames,wavelength=850,ax=None,log_scale=True,show_break=False,get_info=False,**plot_kwargs):
	if ax is None:
	...
	for i,filename in enumerate(filenames):
		...
		if 'abs' in filename:
			...
			if (log_scale):
				...
				ax.plot(new_Av_,new_data_/new_Av_,ls=ls[keys[i]],**plot_kwargs,label='$\\rm a_{max}=%.2f\\, \\mu m$'%(amax))
			x=np.log10(new_Av_)
	...
	if 'abs' in filenames[0]:
		...
		ax.set_xlabel('$A_{\\rm V}\\, (mag.)$')
		ax.set_ylabel('$p_{\\rm ext}/A_{\\rm V}\\,(\\%/mag.)$')
		# ax.set_xlim([1,20])
		try: ## avoid errors if 0.0 or nan or inf values exist for new_Av_
...
@auto_refresh
def plot_pNH(filenames,wavelength=850,ax=None,log_scale=True,show_break=False,get_info=False,**plot_kwargs):
	if ax is None:
		fig,ax=plt.subplots(figsize=(7.5,9))
	# save={}
	# filesave='models.dat'
	...
	for i,filename in enumerate(filenames):
		...
		data=np.loadtxt(filename,skiprows=12)
		NH_=get_NH(filename); new_NH_=np.linspace(NH_.min(),NH_.max(),1000)
		w  =data[:,0]
		x,y=np.shape(data)
		data_=[];I_=[]
		if 'abs' in filename:
		...
		data_=np.array(data_)#; print(len(Av_),len(data_))
		f_ip = interpolate.interp1d(NH_,data_,kind='cubic')
		...
		if 'abs' in filename:
			...
			if (log_scale):
				...
				ax.plot(new_NH_,new_data_/new_NH_,ls=ls[keys[i]],**plot_kwargs,label='$\\rm a_{max}=%.2f\\, \\mu m$'%(amax))
			x=np.log10(new_NH_)
			# x=np.log10(Av_num[idd])
			y=np.log10(new_data_/new_NH_)
			f=interp1d(new_NH_,new_data_/new_NH_)
			# save['Av(amax=%.2f)'%(amax)]=new_Av_
			# save['p/Av(amax=%.2f)'%(amax)]=new_data_/new_Av_
			# output(args,filesave,save).file_save()
			if (show_break):
			...
			sort_indices = np.argsort(NH_)
			NH_sort   = NH_[sort_indices]
			data_sort = data_[sort_indices]
			if (log_scale):
	...
	if 'abs' in filenames[0]:
		...
		# ax.set_xticks([10.0],['10'])
		ax.legend(loc='lower left',bbox_to_anchor=(0.05,0.01),frameon=False)
		ax.set_xlabel('$N({\\rm H_{2}}) ({\\rm cm^{-2}})$')
		ax.set_ylabel('$p_{\\rm ext}/N({\\rm H_{2}})\\,(\\rm \\%/cm^{-2})$')
		# ax.set_xlim([1,20])
		ax.set_ylim([np.min(new_data_/new_NH_)*0.8,np.max(new_data_/new_NH_)*2.0])
		# ax.set_xticks([10.0])
		# ax.set_xticklabels(['10'])
		# ax.set_xticks([1.00])
		# ax.set_xticklabels(['1.0'])
...
def plot_pI(filenames,wavelength,ax=None,show_break=False,get_info=False,**plot_kwargs):
	...
	for i,filename in enumerate(filenames):
		...
		try:
			...
			f_ip = interp1d(I_,data_,kind='linear')
			# f_ip = interp1d(I_,data_,kind='nearest')
